-- Lets a user narrow their daily/weekly recap notification down to a
-- subset of the circle members whose calendar events would otherwise
-- automatically merge in (see shared_circle_cat(..., 'calendar') in
-- send_daily_recap/send_weekly_recap). NULL preserves today's behavior
-- (include everyone visible) so nobody's recap silently changes until
-- they explicitly narrow it via the new picker.
--
-- Applied live via apply_migration as "add_recap_included_uids" and
-- "weekly_recap_included_uids" -- this file mirrors that for the record.

alter table public.profiles
  add column if not exists recap_included_uids uuid[];

create or replace function public.send_daily_recap(p_time_slot text)
returns void
language plpgsql
security definer
