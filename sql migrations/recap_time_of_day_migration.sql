-- Replaces the daily recap's 3-slot morning/noon/evening enum with an
-- exact time-of-day, and adds the same ability to the weekly recap (which
-- previously had no time control at all -- it just rode along on the
-- morning cron's schedule). Backfills existing enum values to sensible
-- clock times before dropping the old column, and moves both crons from
-- fixed hourly slots to a once-a-minute check matching each user's chosen
-- minute -- the only way to support an arbitrary time without hardcoding a
-- new cron per slot.
--
-- Applied live via apply_migration as "recap_time_of_day" and
-- "weekly_recap_time_of_day" -- this file mirrors that for the record.

alter table public.profiles
  add column if not exists daily_recap_time_of_day time not null default '08:00:00',
  add column if not exists weekly_recap_time_of_day time not null default '18:00:00';

update public.profiles set daily_recap_time_of_day = case daily_recap_time
  when 'morning' then '08:00:00'::time
  when 'noon' then '12:00:00'::time
  when 'evening' then '18:00:00'::time
  else '08:00:00'::time
end
where daily_recap_time is not null;

alter table public.profiles drop column if exists daily_recap_time;

DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'send-daily-recap-morning') THEN
    PERFORM cron.unschedule('send-daily-recap-morning');
  END IF;
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'send-daily-recap-noon') THEN
    PERFORM cron.unschedule('send-daily-recap-noon');
  END IF;
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'send-daily-recap-evening') THEN
    PERFORM cron.unschedule('send-daily-recap-evening');
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'send-daily-recap') THEN
    PERFORM cron.schedule('send-daily-recap', '* * * * *', $cron$SELECT send_daily_recap();$cron$);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'send-weekly-recap') THEN
    PERFORM cron.schedule('send-weekly-recap', '* * * * *', $cron$SELECT send_weekly_recap();$cron$);
  END IF;
END $$;
