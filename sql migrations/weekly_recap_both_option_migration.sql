-- Adds a third weekly_recap_relative value, 'both', alongside 'coming' and
-- 'passed' -- one email/notification covering the week that just passed
-- (today-7..today-1) AND the week ahead (today..today+6), each in its own
-- labeled section, instead of having to pick only one direction.
