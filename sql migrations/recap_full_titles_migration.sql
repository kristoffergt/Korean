-- The in-app recap notification body (notifications.body, plain text --
-- the email version already used the full title in its HTML) truncated
-- each event down to just its first word: regexp_replace(split_part(ev.
-- title, ' ', 1), ':$', ''). "Dean's Scholarship Announcement" became just
-- "Dean's". Both send_daily_recap() and send_weekly_recap() now use
-- ev.title directly instead.
--
-- Applied live via apply_migration as "recap_full_titles" -- this file
-- mirrors that for the record. Only the truncation lines changed; every
-- other line is byte-for-byte the same as the live functions before this.
